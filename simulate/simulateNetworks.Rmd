...
{
	dat <- read.csv(paste(size,".csv",sep=""))
	...
	write.csv(dat, paste("truth ",size,".csv",sep=""), row.names=F, quote=F)
...
for(size in SIZE)
	for( sample in NP)
		for(noise in NOISE)
			network.simulation(size = size, sample = sample, noise = noise, outputDir="../data/simulations/")
```