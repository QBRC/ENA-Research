...
```{r}
plot(apply(AUCs[1:3,,,"SPACE",], 4, mean, na.rm=TRUE), ylab="AUC Delta", xlab="Iteration")
points(apply(AUCs[1:3,,,"GeneNet",], 4, mean, na.rm=TRUE), col=2)
points(apply(AUCs[1:3,,,"WGCNA",], 4, mean, na.rm=TRUE), col=3)
```
...
for(size in SIZE[1:4]){
	...
	print(ggplot(AUCtab, aes(Iteration, AUC, color=Method, group=Method)) + geom_point() +  facet_grid(Sigma ~ Samples) + geom_hline(aes(yintercept=0)) + opts(title=size))
}

AUCtab <- as.data.frame.table(AUCs[SIZE[1:4],,,,], responseName="AUC")
AUCtab$Iteration <- as.integer(as.character(AUCtab$Iteration))


AUCavg <- as.data.frame.table(apply(AUCs[SIZE[1:4],,,c("SPACE", "GeneNet"),], c(1,3,4,5), median, na.rm=TRUE), responseName="AUC")
